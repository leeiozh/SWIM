...
bool ImuSensor::writeRegister(uint8_t reg, uint8_t value) {
  wire_->beginTransmission(SwimConfig::MPU_ADDRESS);
  wire_->write(reg);
  wire_->write(value);
...
bool ImuSensor::readRegisters(uint8_t reg, uint8_t *buffer, size_t length) {
  ...
  // Select the uint16_t/size_t overload explicitly. With ESP32 Arduino 2.0.x,
  // the overloaded uint8_t variants produced unreliable burst reads on GPIO43/44.
  const size_t received = wire_->requestFrom(
      static_cast<uint16_t>(SwimConfig::MPU_ADDRESS), length, true);
...
bool ImuSensor::begin(TwoWire &wire) {
  ...
  if (!writeRegister(REG_SMPLRT_DIV, 19)) return false;      // 50 Hz from 1 kHz base
...
bool ImuSensor::read(ImuSample &s, uint64_t monotonicUs) {
  uint8_t d[14];
  if (!readRegisters(REG_ACCEL_XOUT_H, d, sizeof(d))) return false;

  const int16_t axRaw = (int16_t)((d[0] << 8) | d[1]);
  const int16_t ayRaw = (int16_t)((d[2] << 8) | d[3]);
  const int16_t azRaw = (int16_t)((d[4] << 8) | d[5]);
  const int16_t gxRaw = (int16_t)((d[8] << 8) | d[9]);
  const int16_t gyRaw = (int16_t)((d[10] << 8) | d[11]);
  const int16_t gzRaw = (int16_t)((d[12] << 8) | d[13]);

  s.monotonicUs = monotonicUs;
  s.axG = (axRaw / 8192.0f - ACC_BIAS_X) / ACC_SCALE_X;
  s.ayG = (ayRaw / 8192.0f - ACC_BIAS_Y) / ACC_SCALE_Y;
  s.azG = (azRaw / 8192.0f - ACC_BIAS_Z) / ACC_SCALE_Z;
...
bool ImuSensor::calibrateStationary() {
  ...
  for (int i = 0; i < SAMPLE_COUNT; ++i) {
    uint8_t d[14];
    if (readRegisters(REG_ACCEL_XOUT_H, d, sizeof(d))) {
      auto raw = [&](int p) { return (int16_t)((d[p] << 8) | d[p + 1]); };
      const float ax = (raw(0) / 8192.0f - ACC_BIAS_X) / ACC_SCALE_X;
      const float ay = (raw(2) / 8192.0f - ACC_BIAS_Y) / ACC_SCALE_Y;
      const float az = (raw(4) / 8192.0f - ACC_BIAS_Z) / ACC_SCALE_Z;
      const float gx = raw(8) / 65.5f;
      const float gy = raw(10) / 65.5f;
      const float gz = raw(12) / 65.5f;
